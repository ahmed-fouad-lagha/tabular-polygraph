...
def sample_var(vp, u):
    ...
    if d == "normal":
        ...
        result[is_normal] = np.exp(
            mu_ln + sig_ln * stats.norm.ppf(_safe_u(u[is_normal]))
        )
...
def generate_profile(profile_data, n_samples=50_000):
    ...
    U = gen_correlated_uniforms(
        profile_data["correlations"][:n_vars, :n_vars], n_samples
    )
    synth = {}
...
def corr_score(synth, real_corr, vnames):
    ...
    score = 100.0 * (
        ...
        / (np.linalg.norm(real_corr[:n, :n], "fro") + 1e-8)
        * 0.6
    )
    return float(max(0.0, min(100.0, score)))
...
def run_benchmark():
    ...
    for profile_name, profile_data in PROFILES.items():
        ...
        for vname, vp in profile_data["variables"].items():
            ...
            print(
                f"  {vname:<28} {ms:>9.1f}%  {ks:>7.1f}%  {vp['mean']:>12.3g}  {np.mean(sv):>12.3g}  {np.median(sv):>10.3g}  {np.median(sv):>10.3g}{flag}"
            )
    ...
    for pname, r in results.items():
        print(
            f"  {pname:<20} {r['n_variables']:>5} {r['avg_marginal']:>9.2f}% {r['avg_ks']:>7.2f}% {r['correlation']:>7.2f}% {r['overall']:>9.2f}%"
        )
