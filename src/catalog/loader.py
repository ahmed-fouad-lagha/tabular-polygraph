...
DATASETS: dict[str, dict] = {
    ...
    # ── Capital Markets ───────────────────────────────────────────────────────
    "edgar": {
        "name": "SEC EDGAR Financial Statements",
        ...
        "col_count": 13,
        "tags": ["SOC 2 safe", "CSV", "Parquet", "JSON"],
        "fidelity": 90.4,
        "status": "live",
        "use_cases": [
    ...
    "cftc": {
        ...
        "use_cases": [
            ...
            "Options positioning models",
        ],
    },
    "equity_returns": {
        "name": "Equity Returns & Risk Factors",
        "vertical": "Capital Markets",
    ...
    "rental_market": {
        "name": "Residential Rental Market",
        "vertical": "Real Estate",
    ...
    "retail_transactions": {
        ...
        "col_count": 12,
        "tags": ["GDPR safe", "PCI safe", "CSV", "JSON"],
        "fidelity": 91.1,
        "status": "live",
        "use_cases": [
            "Fraud detection",
    ...
    return pd.DataFrame(
        {
            ...
            "property_type": _weighted(
                rng,
                ["Single Family", "Multifamily", "Manufactured", "Condo"],
                [72, 12, 8, 8],
                n,
            ),
            "debt_to_income": dti,
            "state": _weighted(
                rng, states, [14, 12, 10, 8, 5, 5, 4, 4, 4, 4, 4, 4, 4, 3, 3], n
            ),
        }
    )
    ...
    return pd.DataFrame(
        {
            ...
            "total_deposits": (assets * rng.uniform(0.55, 0.80, n))
            .round(0)
            .astype(int),
            ...
            "bank_size_class": _weighted(
                rng, ["community", "regional", "large", "megabank"], [60, 25, 10, 5], n
            ),
            "charter_class": _weighted(
                rng, ["NM", "N", "SM", "SB", "OI"], [35, 30, 18, 12, 5], n
            ),
            "state": _weighted(
                rng,
                ["CA", "TX", "FL", "NY", "IL", "OH", "PA", "NC", "GA", "MI"],
                [14, 12, 10, 9, 7, 6, 5, 5, 4, 4],
                n,
            ),
        }
    )
    ...
    return pd.DataFrame(
        {
            ...
            "loan_term": _weighted(
                rng, [12, 24, 36, 48, 60, 72], [5, 12, 28, 20, 28, 7], n
            ),
            "employment_years": np.clip(rng.exponential(5, n), 0, 40).round(1),
            "default_12m": (
                rng.uniform(0, 1, n) < np.clip(base_default, 0, 0.5)
            ).astype(int),
        }
    )
...
def _build_cftc(n: int = 2000) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        {
            "commodity": _weighted(
                rng, commodities, [10, 12, 8, 9, 7, 9, 8, 6, 7, 8, 8, 8], n
            ),
            "contract_units": _weighted(
                rng,
                ["100 oz", "1000 bbl", "10000 MMBtu", "5000 bu"],
                [25, 25, 25, 25],
                n,
            ),
    ...
    return pd.DataFrame(
        {
            ...
            "realized_vol_21d": np.clip(rng.lognormal(-3.2, 0.6, n), 0.005, 0.15).round(
                4
            ),
            "beta": beta,
    ...
    return pd.DataFrame(
        {
            ...
            "rating_agency": _weighted(
                rng, ["Moody's", "S&P", "Fitch"], [40, 40, 20], n
            ),
            "sector": _weighted(
                rng,
                [
                ...
                [25, 20, 12, 11, 10, 10, 12],
                n,
            ),
            "subordination": _weighted(
                rng,
                [
                    "Senior Secured",
                    "Senior Unsecured",
            ...
            "default_flag": (
                rng.uniform(0, 1, n) < np.where(ig_flag, 0.001, 0.04)
            ).astype(int),
...
def _build_fred_macro(n: int = 2000) -> pd.DataFrame:
    ...
    gdp = np.where(recession, rng.normal(-3.5, 1.5, n), rng.normal(2.5, 1.2, n)).round(
        2
    )
    ...
    return pd.DataFrame(
        {
            ...
            "industrial_production": np.clip(
                rng.normal(1.8, 4.5, n), -16.0, 12.0
            ).round(2),
    ...
    return pd.DataFrame(
        {
            ...
            "ownership": _weighted(
                rng,
                ["Private", "Federal Govt", "State Govt", "Local Govt"],
                [75, 3, 7, 15],
                n,
            ),
            "state": _weighted(
                ...
                [14, 12, 10, 9, 6, 6, 5, 5, 4, 4],
                n,
            ),
            "avg_weekly_wage": _lognorm(rng, 6.6, 0.4, 400, 3500, n)
            .round(0)
            .astype(int),
    ...
    return pd.DataFrame(
        {
            ...
            "current_account_pct_gdp": np.clip(rng.normal(-2.1, 5.5, n), -25, 20).round(
                2
            ),
...
def _build_central_bank_rates(n: int = 2000) -> pd.DataFrame:
    ...
    country = _weighted(
        rng, countries, [12, 11, 9, 8, 8, 6, 6, 5, 4, 4, 6, 5, 7, 5, 4], n
    )
    ...
    return pd.DataFrame(
        {
            ...
            "reserve_requirement": np.clip(rng.lognormal(1.5, 0.7, n), 0.0, 20.0).round(
                2
            ),
            ...
            "exchange_rate_regime": _weighted(
                rng,
                ["Free Float", "Managed Float", "Peg", "Currency Board"],
                [45, 30, 15, 10],
                n,
            ),
    ...
    return pd.DataFrame(
        {
            ...
            "filing_status": _weighted(
                rng, ["Single", "MFJ", "MFS", "HoH", "QW"], [44, 38, 2, 12, 4], n
            ),
            ...
            "capital_gains": np.where(
                rng.uniform(0, 1, n) > 0.6,
                (agi * rng.uniform(0, 0.4, n)).astype(int),
                0,
            ),
            "business_income": np.where(
                rng.uniform(0, 1, n) > 0.7,
                (agi * rng.uniform(0, 0.3, n)).astype(int),
                0,
            ),
    ...
    return pd.DataFrame(
        {
            ...
            "employment_status": _weighted(
                rng, ["Employed", "Unemployed", "Not in labor force"], [58, 5, 37], n
            ),
            ...
            "education": _weighted(
                rng,
                ["No HS", "HS Grad", "Some College", "Bachelor", "Graduate"],
                [12, 27, 28, 21, 12],
                n,
            ),
        }
    ...
    return pd.DataFrame(
        {
            ...
            "ibnr_estimate": _lognorm(rng, 6.5, 1.5, 0, 10_000_000, n)
            .round(0)
            .astype(int),
            ...
            "policy_type": _weighted(
                rng, ["Commercial", "Personal", "Specialty"], [55, 35, 10], n
            ),
        }
    )
...
def _build_life_insurance(n: int = 2000) -> pd.DataFrame:
    ...
    mort_rate = np.clip(
        mort_base * (1 + 0.8 * smoker) + rng.exponential(0.0002, n), 0.0001, 0.25
    )
    lapse_rate = np.clip(
        0.12 * np.exp(-0.15 * duration) + rng.exponential(0.02, n), 0.005, 0.35
    )
    face = _lognorm(rng, 11.8, 1.0, 10_000, 10_000_000, n).astype(int)
    return pd.DataFrame(
        {
            "face_amount": face,
            "annual_premium": (face * mort_rate * rng.uniform(1.2, 2.0, n))
            .round(0)
            .astype(int),
            "policy_duration": duration,
            "age_at_issue": age,
            "underwriting_class": _weighted(
                rng,
                [
                    "Preferred Plus",
                    "Preferred",
            ...
            "surrender_value": (
                face * np.clip(duration / 40, 0, 0.9) * rng.uniform(0.7, 1.0, n)
            )
            .round(0)
            .astype(int),
            "cash_value": (
                face * np.clip(duration / 50, 0, 0.8) * rng.uniform(0.5, 0.9, n)
            )
            .round(0)
            .astype(int),
            "product_type": _weighted(
                rng,
                ["Term", "Whole Life", "Universal Life", "Variable UL", "Indexed UL"],
                [35, 25, 20, 10, 10],
                n,
            ),
    ...
    return pd.DataFrame(
        {
            ...
            "market_tier": _weighted(
                rng, ["Tier 1", "Tier 2", "Tier 3"], [35, 40, 25], n
            ),
            "submarket": _weighted(
                rng,
                ["CBD", "Suburban", "Urban Fringe", "Secondary"],
                [30, 38, 18, 14],
                n,
            ),
    ...
    return pd.DataFrame(
        {
            ...
            "rent_to_income": np.clip(
                (gross_rent * 12) / np.maximum(metro_income, 1) * 100, 10, 80
            ).round(1),
            "yoy_rent_change": np.clip(rng.normal(4.5, 6.0, n), -15, 25).round(1),
            "unit_type": _weighted(
                rng, ["Studio", "1BR", "2BR", "3BR", "4BR+"], [12, 28, 35, 18, 7], n
            ),
            "bedrooms": _weighted(rng, [0, 1, 2, 3, 4], [12, 28, 35, 18, 7], n),
            "year_built_band": _weighted(
                rng,
                ["Pre-1960", "1960-1980", "1980-2000", "2000-2015", "Post-2015"],
                [18, 22, 25, 22, 13],
                n,
            ),
            "metro_tier": _weighted(
                rng, ["Tier 1", "Tier 2", "Tier 3", "Tier 4"], [20, 30, 30, 20], n
            ),
            "state": _weighted(
                rng,
                ["CA", "TX", "FL", "NY", "PA", "IL", "OH", "GA", "NC", "MI"],
...
def _build_retail_transactions(n: int = 2000) -> pd.DataFrame:
    ...
    channel = _weighted(
        rng,
        ["Card Present", "Card Not Present", "ACH", "Wire", "Mobile", "ATM"],
        [35, 28, 14, 5, 12, 6],
        n,
    )
    fraud_base = np.where(
        channel == "Card Not Present", 0.003, np.where(channel == "ATM", 0.002, 0.0006)
    )
    return pd.DataFrame(
        {
            "amount": amount,
            "channel": channel,
            "merchant_category": _weighted(
                rng, mcc, [18, 14, 10, 16, 7, 8, 7, 6, 5, 9], n
            ),
            "day_of_week": _weighted(
                rng,
                ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"],
                [13, 13, 14, 15, 17, 16, 12],
                n,
            ),
            "hour_of_day": rng.integers(0, 24, n),
            "transaction_type": _weighted(
                rng,
                ["Purchase", "Refund", "Transfer", "Bill Payment", "Cash"],
                [70, 5, 10, 10, 5],
                n,
            ),
    ...
    return pd.DataFrame(
        {
            ...
            "realized_vol_21d": np.clip(
                rng.lognormal(-2.8, 0.55, n), 0.005, 0.12
            ).round(4),
            ...
            "dollar_index_return": np.clip(
                rng.normal(0.0001, 0.004, n), -0.04, 0.04
            ).round(4),
