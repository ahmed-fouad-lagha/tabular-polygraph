...
**Resolution:** Remedy 1 implemented as the primary construct-validity study (Question 1, §4.1, Table~\ref{tab:identity_verification}): on both constrained datasets across all four generators ($N=10$ seeds), records flagged by HIF ($H < 0.5$) are independently confirmed as identity-violating in $\ge 98.9\%$ of cases (1.000 in six of eight configurations) using arithmetic identities that hold to $\approx 10^{-13}$ in the real data. Remedy 3 partially implemented: the Terminology paragraph at the start of §4 defines *integrity* (consistency with the *learned* conditional manifold) vs. *validity* (true possibility), and §4.1 now quantifies the flagged cohort as externally invalid at near-universal rates; the abstract/Fig. 1 retain "hallucination/impossible junction" as interpretive shorthand now anchored by the §4.1 confirmation rates. Remedy 2 (out-of-support flagging) was not run; the direct confirmation rates of Remedy 1 make it redundant for the construct-validity question.
...
**Resolution:** CIs now reported for all four positive effects in §5.1 (Census CTGAN $[+0.049, +0.160]$; Census Vine $[+0.170, +0.220]$; Purchases CTGAN $[+0.010, +0.245]$; Purchases Vine $[+0.306, +0.493]$). The abstract and §1 Summary of Contributions now carry explicit CIs plus a Bonferroni sensitivity sentence: the $+12.8$ Purchases-CTGAN effect ($p = 0.037$, CI $[+0.010, +0.245]$) is flagged as not surviving Bonferroni correction across 16 configurations, and the $+0.104$ Census-CTGAN effect is reported with its wide CI. The negative effect (Credit Gaussian, $-0.021$, $p = 0.013$) remains disclosed in §5.1.
...
**Resolution:** The Rule-only baseline is now reported in §5.1: pruning records that violate mined implication rules ($\approx 3\%$ of the cohort; retention $97.0\%$) leaves utility unchanged (Census ACS CTGAN: F1 $0.426$ vs.\ $0.427$ unfiltered; Gaussian Copula: $0.937$ vs.\ $0.940$), whereas full HIF filtering raises CTGAN F1 to $0.532$ at $65.7\%$ retention. The text concludes explicitly that the utility recovery is driven by the LSE/NIC conditional-dependency layers rather than by hard-rule pruning alone.
...
**Resolution:** Abstract reframed from "two primary contributions" to "an integrative contribution"; §1 changed from "we introduce" to "we present … an integrative design that composes established building blocks …; its contribution lies in the problem formulation (per-record joint-conditional plausibility with record-level attribution) and in the systematic validation of when such auditing adds utility." The Valiant/Pensia/Neykov/Paninski sample-complexity citations are now operationalized as a bound on HIF's own per-cell reliability (§3, expanded in Appendix~\ref{app:sample_complexity}).
...
**Resolution:** The Discussion (Practical Interpretation subsection) now carries an explicit deployment-facing statement: at the default $H < 0.5$ frontier, filtering improves utility only when the violation→target association is strong (Census ACS CTGAN/Vine; Online Purchases CTGAN/Vine); otherwise it is utility-neutral or harmful (Credit Gaussian degradation $p = 0.013$); and retention below $\approx 15\%$ should trigger manual review before deployment. The 4/16 selective-recovery caveat and cohort-shrinkage figures are stated alongside, and §5.1 quantifies the operating-point boundary via the threshold sweep (Table~\ref{tab:threshold_utility}).
...
1. **"Violation rate" is used with two meanings.** In Table 2 (cross-architecture) it means "fraction of synthetic records flagged by HIF"; in Table 5 (ablation) the column is labeled "Violation Rate" but contains the *rejection* rate (e.g., "No filtering 0.0%"). Consider renaming the ablation column "% Flagged / Retention loss," and note that "No filtering — 0.0%" is trivially true.
   **Resolution:** The ablation column is renamed **"Filtered Out (%)"** and the "No filtering" row now reads `---` instead of a trivially true 0.0%. Ablation prose was reworded accordingly ("flag few records (2.2% filtered out)", "13.5% filtered out", "filtered-out rate $<1\%$"), and the subsection title is now "Why Adding Components Lowers the Flag Rate." Table 2's "Viol. Rate" is retained with its established meaning (fraction flagged by HIF), which §4.1 now distinguishes from the externally confirmed violation rate of the identity-verification study.
2. **PR-AUC is reported once** (§6, dependency violations: 0.789) despite §5.1 declaring threshold-independent ranking quality a primary metric. Report PR-AUC for all five error families in Table 4.
   **Resolution:** Already satisfied — the held-out Table 4 (Panel A) reports PR-AUC for all five error families (dependency violations, covariate shift, feature dropout, row duplication, random noise), including the GMM learned-density baseline.
3. **Missing CIs** for the Purchases effects ($+0.128$, $+0.399$) in the text; the paper reports CIs for the Census significance tests — do the same here (see M2).
   **Resolution:** Covered by M2 — §5.1 now reports CIs for all four positive effects, including Online Purchases CTGAN $[+0.010, +0.245]$ and Vine $[+0.306, +0.493]$; the abstract and §1 carry the Purchases-CTGAN CI explicitly.
4. **Fig. 1 caption** says "KS ≈ 0.96" for Gaussian Copula 1D marginals of Quantity/Total; Table 2 reports dataset-level KS $0.903 \pm 0.006$ for that configuration. These are different quantities (per-column vs. aggregate) and likely consistent, but the caption should say so to avoid a reviewer "contradiction" flag.
   **Resolution:** Already satisfied — the Fig. 1 caption now reads "per-column KS $\approx 0.96$ for these two marginals, versus the dataset-level KS of $0.903 \pm 0.006$ reported in Table~\ref{tab:architecture}", explicitly disambiguating the two quantities.
5. **§3 (Related Work) is one wall of text**; the "Evaluation Metrics and the Dependency Gap" paragraph is a single multi-claim paragraph of several hundred words. Split into 2–3 paragraphs.
   **Resolution:** §3 is split into three paragraphs — "Evaluation Metrics and the Dependency Gap," "Sample-Complexity Limits on Aggregate Auditing" (new, connecting the Valiant/Pensia/Neykov/Paninski bounds to HIF's own per-cell reliability, also resolving the M4 positioning point), and a closing paragraph on HIF-style automation.
...
8. **Online Purchases TVAE** (Table 3) reports F1 $0.889$ at 1.1% retention with an N/A in the HIF column; the $0.889$ itself is estimated on a 1.1% cohort and is as unreliable as the HIF column — mark both N/A for consistency.
   **Resolution:** In the utility-filtering table, the Online Purchases TVAE row now reports `N/A*` in both the full and filtered F1 columns (retention 1.1%). The same applies to the Supermarket Sales CTGAN/TVAE/Vine rows, whose retained cohorts are 0.0–0.1% and likewise too small to estimate utility.
