...
**Resolution:** §5.1 now ends with an explicit deployment-guidance statement: at the default $H < 0.5$ frontier, filtering improves utility only when the violation→target association is strong (Census ACS CTGAN/Vine; Online Purchases CTGAN/Vine); otherwise it is utility-neutral or harmful (Credit Gaussian degradation $p = 0.013$); and retention below $\approx 15\%$ should trigger manual review before deployment. The 4/16 selective-recovery caveat and cohort-shrinkage figures remain stated in the abstract's terms via the CI/boundary reporting above.
