...
\paragraph{3. Quantitative Decoupling Evidence.} Pearson correlation analysis across all 16 dataset--generator configurations is consistent with the decoupling: HIF shows no linear relationship with marginal fidelity ($\rho(\text{HIF}, \text{KS}) = 0.008$) and only a weak relationship with support coverage ($\rho(\text{HIF}, \alpha) = 0.275$), while exhibiting strong negative correlation with the Joint Correlation Distance ($\rho(\text{HIF}, \text{JCD}) = -0.83$). This indicates that HIF evaluates a different axis of synthetic data quality than marginal distributional metrics.
