...
To audit categorical consistency, we first identify the set of Dependency Hubs $\mathcal{H} \subset \{1, \dots, K_{hub}\}$ that govern the structural dependencies of the dataset. We define the importance of a feature $h$ using the Feature Dependency Score $S(h)$, which formalizes the predictability of feature $x_h$ from the remaining features $\mathbf{x}_{\setminus h}$ using an out-of-bag random forest ensemble $f_h$. Formally, this is the expected classification accuracy:
\begin{equation}
    S(h) = \mathbb{E}_{\mathbf{x} \sim \mathcal{D}_{real}} \left[ \mathbb{I} \left( \arg\max_{c \in \mathcal{X}_h} f_h(x_h=c \mid \mathbf{x}_{\setminus h}) = x_h \right) \right]
\end{equation}
where $\mathbb{I}$ is the indicator function and $f_h(x_h=c \mid \mathbf{x}_{\setminus h})$ is the ensemble's estimated conditional probability for class $c$. By maximizing $S(h)$, we select features that are most strongly constrained by conditional interactions with remaining attributes. For each hub $h \in \mathcal{H}$, the sentinel $f_h: \mathcal{X}_{\setminus h} \to [0, 1]^{|\mathcal{X}_h|}$ learns the conditional density $P(x_h \mid \mathbf{x}_{\setminus h})$.

\paragraph{Confidence Floor Calibration.}
To ensure robustness against categorical noise, we establish a data-driven Confidence Floor $\delta_h$ for each hub. We define $\delta_h$ using the empirical 5th-percentile ($Q_{0.05}$) of the probability mass assigned to the correct category by the Sentinel in the ground-truth data, with a safety minimum of 0.01:
\begin{equation}
    \delta_h = \max \left( Q_{0.05} \Big( \{ f_h(x_h \mid \mathbf{x}_{\setminus h}) \mid \mathbf{x} \in \mathcal{D}_{real} \} \Big), 0.01 \right)
\end{equation}
This 5th-percentile floor acts as a conservative safety margin, ensuring that the HIF does not penalize records that fall within the natural density of the ground truth manifold. Although sensitive to sample volume, the use of a quantile-based anchor provides a non-parametric baseline that automatically adapts to the varying conditional entropy of different dependency hubs.
...
\For{each continuous feature $y$}
    ...
    \State $\tau_y \leftarrow \max(Q_{0.98}(R_y), \text{Median}(R_y) + 2 \cdot \text{MAD}(R_y))$ \Comment{Robust Threshold}
...
\begin{equation}
    \tau_y = \max(Q_{0.98}(R_y), \text{Median}(R_y) + 2 \cdot \text{MAD}(R_y))
\end{equation}
...
The results demonstrate that the Hybrid Integrity Framework provides a ``strictly conservative'' diagnosis. By employing a Multiplicative Integrity Model, HIF ensures that a failure in any single semantic dimension results in a low integrity score. This non-compensatory evaluation is critical for high-stakes domains, where a single inconsistent record can invalidate an analysis.
