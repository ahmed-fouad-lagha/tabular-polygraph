...
\midrule
\textbf{Purchases}    & Gaussian Copula & Full synthetic & 100.0 & 0.980 $\pm$ 0.012 & -- \\
                      & Gaussian Copula & \textbf{HIF Filtered} & \textbf{58.4} & 0.976 $\pm$ 0.014 & -- \\
\bottomrule
\end{tabular}
\end{table}

To characterize when integrity filtering recovers utility across domains, we tested the paired F1 difference (HIF-filtered minus full synthetic) across all dataset--generator combinations of the cross-architecture benchmark with $N=10$ seeds. The gains are selective and mirror the integrity diagnostics: significant recovery appears exactly where HIF flags structural errors, on Census ACS ($\Delta\text{F1} = +0.183$, $p = 0.0003$ for CTGAN; $+0.205$, $p < 0.001$ for Vine Copula) and Online Purchases ($\Delta\text{F1} = +0.164$, $p = 0.011$ for CTGAN; $+0.413$, $p < 0.001$ for Vine Copula). In contrast, cohorts diagnosed as high-integrity show no measurable change (Census ACS Gaussian Copula, $p = 0.29$; Online Purchases Gaussian Copula, $p = 0.27$; Adult Gaussian Copula, $p = 0.82$), confirming that HIF does not penalize well-formed synthetic data. Two configurations show small but significant degradations (Adult TVAE, $\Delta\text{F1} = -0.005$, $p = 0.038$; Credit Gaussian Copula, $\Delta\text{F1} = -0.025$, $p < 0.001$), indicating that aggressive pruning can remove informative minority records when the cohort is already largely clean; this remains the exception rather than the rule. Finally, four transaction configurations (Supermarket Sales CTGAN/TVAE/Vine; Online Purchases TVAE) retain fewer than 1\% of records, leaving the retained cohort too small to estimate utility reliably, and their F1 deltas are consequently not reported. Together these results indicate that HIF-driven filtering is a targeted remediation: it recovers utility when structural errors are present and is utility-neutral when they are not.
...
\paragraph{1. Integrity-Fidelity Decoupling on Constrained Domains.} Across all four datasets, Vine Copulas achieve high marginal statistical fit ($\text{KS} \ge 0.969$). However, on datasets with cross-feature relational constraints (Supermarket Sales and Online Purchases), Vine Copulas exhibit severe joint dependency failures, with violation rates of \textbf{85.9\% to 99.9\%} ($\text{HIF} \le 0.273$). CTGAN and TVAE exhibit similar dependency deficits on transaction constraints ($\text{HIF} = 0.02\text{--}0.27$, violation rates 85.6\%--100\%). Marginal metrics (KS) report these models as high-fidelity, underscoring that aggregate fidelity metrics do not capture joint dependency ruptures.

\paragraph{2. Evaluation on Unconstrained Manifolds.} On unconstrained demographic and financial benchmarks (Adult Income, Credit Default), raw generators preserve conditional structures better, with HIF scores ranging from 0.32 to 0.98 (violation rates 0.6\%--69.7\%). In these regimes, HIF provides a continuous gradient of structural integrity, allowing practitioners to compare generator maturity. Notably, TVAE achieves the highest HIF on both Credit Default (0.838) and Adult Income (0.980), demonstrating that VAE-based architectures can excel at preserving joint dependencies on unconstrained domains.

\paragraph{3. Quantitative Decoupling Evidence.} Pearson correlation analysis across all 16 dataset--generator configurations of the four audit benchmarks (Census ACS is held out as the calibration benchmark; Table~\ref{tab:architecture}) characterizes the decoupling precisely: HIF shows no linear relationship with marginal fidelity ($\rho(\text{HIF}, \text{KS}) = 0.022$) and only a weak relationship with support coverage ($\rho(\text{HIF}, \alpha) = 0.305$). In contrast, HIF exhibits a strong positive correlation with joint correlation fidelity ($\rho(\text{HIF}, \text{JCD}) = +0.82$). This indicates that HIF aligns with joint-dependency structure while being decoupled from per-column marginal distributional metrics: high marginal fidelity (KS) does not imply high integrity, exactly the pattern observed for Vine Copulas on constrained domains.
...
\quad Gaussian Copula & 0.903 $\pm$ 0.006 & 0.892 $\pm$ 0.012 & 0.918 $\pm$ 0.013 & \textbf{0.963 $\pm$ 0.005} & \textbf{2.6 $\pm$ 0.6\%} \\
\quad Vine Copula     & \textbf{0.975 $\pm$ 0.003} & 0.792 $\pm$ 0.011 & 0.862 $\pm$ 0.016 & 0.026 $\pm$ 0.002 & 99.9 $\pm$ 0.1\% \\
\quad CTGAN           & 0.801 $\pm$ 0.026 & 0.678 $\pm$ 0.061 & 0.881 $\pm$ 0.016 & 0.024 $\pm$ 0.010 & 100.0 $\pm$ 0.0\% \\
\quad TVAE            & 0.901 $\pm$ 0.021 & 0.866 $\pm$ 0.047 & 0.963 $\pm$ 0.026 & 0.022 $\pm$ 0.001 & 100.0 $\pm$ 0.0\% \\
\midrule
\multicolumn{6}{l}{\textbf{Online Purchases}} \\
\quad Gaussian Copula & 0.753 $\pm$ 0.010 & 0.488 $\pm$ 0.013 & 0.823 $\pm$ 0.026 & \textbf{0.629 $\pm$ 0.010} & \textbf{41.6 $\pm$ 2.4\%} \\
\quad Vine Copula     & \textbf{0.969 $\pm$ 0.004} & 0.831 $\pm$ 0.019 & 0.943 $\pm$ 0.011 & 0.273 $\pm$ 0.011 & 85.9 $\pm$ 1.2\% \\
\quad CTGAN           & 0.702 $\pm$ 0.056 & 0.554 $\pm$ 0.064 & 0.889 $\pm$ 0.059 & 0.271 $\pm$ 0.023 & 85.6 $\pm$ 3.3\% \\
\quad TVAE            & 0.869 $\pm$ 0.017 & 0.731 $\pm$ 0.021 & 0.801 $\pm$ 0.040 & 0.120 $\pm$ 0.010 & 99.3 $\pm$ 0.4\% \\
\midrule
\multicolumn{6}{l}{\textbf{Credit Default}} \\
\quad Gaussian Copula & 0.747 $\pm$ 0.002 & 0.729 $\pm$ 0.009 & 0.951 $\pm$ 0.009 & 0.612 $\pm$ 0.006 & 30.6 $\pm$ 0.9\% \\
\quad Vine Copula     & \textbf{0.981 $\pm$ 0.001} & 0.801 $\pm$ 0.011 & 0.813 $\pm$ 0.014 & 0.402 $\pm$ 0.009 & 55.6 $\pm$ 1.5\% \\
\quad CTGAN           & 0.730 $\pm$ 0.074 & 0.584 $\pm$ 0.058 & 0.868 $\pm$ 0.019 & 0.316 $\pm$ 0.033 & 69.7 $\pm$ 4.1\% \\
\quad TVAE            & 0.884 $\pm$ 0.007 & 0.961 $\pm$ 0.013 & 0.919 $\pm$ 0.013 & \textbf{0.838 $\pm$ 0.008} & \textbf{12.8 $\pm$ 1.0\%} \\
\midrule
\multicolumn{6}{l}{\textbf{Adult Income}} \\
\quad Gaussian Copula & 0.848 $\pm$ 0.006 & 0.939 $\pm$ 0.006 & 0.838 $\pm$ 0.016 & 0.829 $\pm$ 0.009 & 10.8 $\pm$ 1.0\% \\
\quad Vine Copula     & \textbf{0.981 $\pm$ 0.005} & \textbf{0.964 $\pm$ 0.008} & 0.798 $\pm$ 0.022 & 0.761 $\pm$ 0.005 & 16.6 $\pm$ 1.0\% \\
\quad CTGAN           & 0.774 $\pm$ 0.052 & 0.825 $\pm$ 0.071 & 0.817 $\pm$ 0.022 & 0.673 $\pm$ 0.014 & 28.1 $\pm$ 1.5\% \\
\quad TVAE            & 0.907 $\pm$ 0.015 & 0.692 $\pm$ 0.035 & 0.752 $\pm$ 0.017 & \textbf{0.980 $\pm$ 0.003} & \textbf{0.6 $\pm$ 0.4\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Quantitative Privacy Audit.} To address the privacy implications of integrity filtering, we conducted a Membership Inference Attack (MIA) audit using distance-to-closest-record (DCR) shadow modeling (Appendix~\ref{app:privacy}). The AUC values remained near random-guessing behavior for both the full synthetic cohort ($AUC = 0.490 \pm 0.026$) and the HIF-filtered cohort ($AUC = 0.499 \pm 0.041$), indicating no clear membership leakage signal introduced by integrity filtering in this setup, even when measured against the stringent standards of modern differential privacy \cite{dwork2006}.
