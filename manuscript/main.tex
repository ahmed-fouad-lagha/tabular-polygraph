...
\paragraph{Summary of Technical Novelty vs. Prior Literature.} To ensure total clarity regarding our technical contributions, we explicitly demarcate literature-derived building blocks from our original contributions:
\begin{itemize}
    \item \textit{Literature-Derived Components:} Base supervised estimators (Random Forests \cite{breiman2001random}, Gradient Boosted Regressors \cite{friedman2001greedy}), spectral categorical embeddings via SVD/MCA \cite{abdi2010principal}, distribution-free quantile error bounds \cite{shafer2008tutorial}, and robust percentile dispersion thresholds \cite{tukey1977exploratory} are standard statistical tools.
    \item \textit{Primary Original Contributions:} (1) Automated, domain-agnostic discovery of latent Dependency Hubs via predictive synergy $S(h)$; (2) Non-compensatory multiplicative product aggregation ($\prod_l (1 - \mathcal{P}_l)$) acting as a Logical AND gate to prevent marginal fidelity from masking localized row violations; (3) Formalization and empirical audit of joint conditional hallucinations (``impossible junctions'') where individual features are unexceptional; and (4) Empirical proof that HIF row filtering recovers downstream predictive utility (+1.9\% F1) while preserving differential privacy bounds.
\end{itemize}
...
In contrast to additive fidelity metrics that allow a model to ``compensate'' for logical failures with high statistical overlap, HIF formalizes integrity as an Algebraic Intersection. Drawing from the structured drift-proof framework \cite{guo2026structured}, the hybrid integrity score $H(\mathbf{x})$ is defined as the strict multiplicative product of the component validities:
\begin{equation}
    H(\mathbf{x}) = \prod_{l \in \{cat, cont, rule\}} (1 - \mathcal{P}_l(\mathbf{x}))
\end{equation}
...
\begin{enumerate}
    ...
    \item \textbf{Continuous Audit (NIC):} SVD projects $[\text{Age}=14, \text{Education}=\text{PhD}]$ to latent vector $\mathbf{z}$. The regressor $r_y$ predicts expected salary $\hat{y} = \$12,000$ with tolerance bandwidth $\gamma_y = \$5,000$. For a hallucinated salary $y = \$150,000$, the continuous penalty evaluates to $\mathcal{P}_y = \text{clip}\left(\frac{|150,000 - 12,000| - 5,000}{5,000}, 0, 1\right) = 1.0$.
